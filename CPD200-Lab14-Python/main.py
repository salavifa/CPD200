...
class MainPage(_BaseHandler):
    def get(self):
        ...
        if contribStats is None:
          query = models.LeaderboardStats.query()
          query = query.filter(models.LeaderboardStats.statType == 'contribution')
          query = query.order(-models.LeaderboardStats.value)
        ...
        if scoreStats is None:
          query = models.LeaderboardStats.query()
          query = query.filter(models.LeaderboardStats.statType == 'score')
          query = query.order(-models.LeaderboardStats.value)
        ...
        if systemStats is None:
            query = models.SystemStats.query()
            query.order(-models.SystemStats.created)
            systemStats = query.fetch(1)
            
            memcache.add('systemStats', systemStats)
            
        self.template_values['systemStats'] = systemStats[0] 
...
class CompleteAchievement(_BaseHandler):
  def get(self):
    ...
    task = taskqueue.add(
            url='/tasks/GenerateSystemStats',
            queue_name='statqueue',
            method='GET'
            )   
